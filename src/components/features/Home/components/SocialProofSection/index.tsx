        ...
        >
          {stats.map((stat) => (
            <motion.div
              ...
              className="group flex flex-col items-center rounded-2xl border border-reviuy-gray-200/60 bg-white p-6 shadow-sm transition-all duration-300 hover:border-reviuy-gray-300 hover:shadow-lg md:p-8"
            >
              <div className={`mb-4 rounded-xl ${stat.bgColor} p-3 transition-transform duration-300 group-hover:scale-110`}>
                <stat.icon className={`h-6 w-6 ${stat.color}`} />
              </div>
              <span className={`${playfair.className} text-2xl font-bold text-reviuy-gray-900 md:text-3xl`}>
                {stat.value}
              </span>
              <span className={`${manrope.className} mt-1 text-center text-sm text-reviuy-gray-500`}>
                {stat.label}
              </span>
            </motion.div>
          ))}
