...
'Vista pública de reviews sin user_id para proteger anonimato de usuarios';
...
comment on view public.reviews_with_votes_public is 
'Vista pública de reviews con estadísticas de votos, sin user_id';

alter view public.reviews_with_votes_public set (security_invoker = on);
...
comment on view public.real_estate_reviews_public is 
'Vista pública de reseñas de inmobiliarias sin user_id';

alter view public.real_estate_reviews_public set (security_invoker = on);
...
left join lateral (
  select
    ...
    count(*) filter (where vote_type = 'dislike') as dislikes,
    count(*) as total_votes
  from public.real_estate_review_votes rerv
  where rerv.real_estate_review_id = rer.id
) stats on true
where rer.deleted_at is null;

comment on view public.real_estate_reviews_with_votes_public is 
'Vista pública de reseñas de inmobiliarias con votos, sin user_id';

alter view public.real_estate_reviews_with_votes_public set (security_invoker = on);
...
create or replace function public.get_user_vote_on_review(
  p_review_id uuid
)
returns text
language sql
...
comment on function public.get_user_vote_on_review is 
'Obtiene el voto del usuario autenticado en una review específica';
...
create or replace function public.get_user_vote_on_real_estate_review(
...
comment on function public.get_user_vote_on_real_estate_review is 
'Obtiene el voto del usuario autenticado en una reseña de inmobiliaria';
