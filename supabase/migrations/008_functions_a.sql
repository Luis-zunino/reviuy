...
do $do$ BEGIN IF EXISTS (
    ...
    ) THEN DROP TRIGGER IF EXISTS cleanup_rate_limits_trigger ON public.rate_limits;

CREATE TRIGGER cleanup_rate_limits_trigger
AFTER
INSERT
    ON public.rate_limits FOR EACH ROW EXECUTE FUNCTION cleanup_rate_limits_on_insert();

RAISE NOTICE 'Trigger cleanup_rate_limits_trigger recreado';

ELSE RAISE WARNING 'Tabla rate_limits no existe aún. El trigger se creará cuando la tabla sea creada.';
...
ELSE RAISE WARNING 'Table rate_limits does not exist yet. View will be created when table is created.';
...
create or replace function update_updated_at_column () RETURNS TRIGGER as $function$ BEGIN IF NEW IS DISTINCT FROM OLD THEN
    NEW.updated_at = now();
...
create or replace function update_real_estate_counters () RETURNS TRIGGER as $function$ BEGIN IF (TG_OP = 'INSERT') THEN IF NEW.real_estate_id IS NOT NULL THEN
UPDATE
    public.real_estates
SET
    updated_at = NOW()
