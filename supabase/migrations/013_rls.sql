...
create policy real_estate_reviews_update_own on public.real_estate_reviews for
update using (user_id = auth.uid ())
with
  check (user_id = auth.uid ());
...
CREATE POLICY real_estate_votes_select_public ON real_estate_votes FOR
SELECT
  USING (true);
...
CREATE POLICY real_estate_votes_update_own ON real_estate_votes FOR
...
CREATE POLICY real_estate_review_votes_select_public ON real_estate_review_votes FOR
SELECT
  USING (true);
...
CREATE POLICY real_estate_review_votes_update_own ON real_estate_review_votes FOR
UPDATE USING (auth.uid () = user_id)
WITH
  CHECK (auth.uid () = user_id);
