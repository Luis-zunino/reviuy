...
test.describe('Address — página de detalle por dirección', () => {
  test('carga con los elementos principales para Montevideo', async ({ page }) => {
    ...
    await expect(page.getByText('Montevideo', { exact: false }).first()).toBeVisible({
      timeout: 15000,
    });
    ...
    // Debe mostrar el botón para crear reseña
    await expect(
      page.getByRole('button', { name: 'Compartí tu experiencia' }).first()
    ).toBeVisible();
  });
  ...
  test('muestra mensaje de estado vacío cuando no hay reseñas', async ({ authPage }) => {
    ...
    await expect(authPage.getByText('Montevideo', { exact: false }).first()).toBeVisible({
      timeout: 15000,
    });
